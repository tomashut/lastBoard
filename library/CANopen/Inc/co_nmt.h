...
/*! \brief  Set CANopen Node Mode.
 *
 *  This function sends a NMT (Network Management) message to set the mode
 *  of a specific CANopen node.
 *
 * \param cif
 *    Pointer to the CAN interface.
 * \param CS
 *    Command specifier for the NMT message.
 * \param nodeID
 *    Node ID of the target CANopen node.
 * \return
 *    One of the error codes in \ref CO_ERR
 */
CO_ERR CONmtSetNodeMode(CO_IF *cif, uint8_t CS, uint8_t nodeID);
